...
app.use(helmet({
    contentSecurityPolicy: {
        directives: {
            ...
            scriptSrc: ["'self'"],
            imgSrc: ["'self'", "data:", "https:", "https://fonts.gstatic.com"], // Modificado para permitir fuentes
            fontSrc: ["'self'", "https://fonts.gstatic.com"] // Añadido para fuentes
        }
    }
}));
...
const quejaLimiter = rateLimit({
    ...
    max: 3, // Máximo 3 quejas por minuto por IP
...
app.post('/api/login', loginLimiter, async (req, res) => {
    ...
    try {
        ...
        await pool.query('INSERT INTO refresh_tokens (user_id, token_hash, expires_at) VALUES ($1, $2, $3)', [user.id, hashedRefreshToken, expiresAt]);
...
app.post('/enviar-queja', quejaLimiter, async (req, res) => {
    ...
    if (error) {
        ...
        logger.warn(`Intento de envío de queja con datos inválidos: ${errores}`, { ip: req.ip, body: req.body });
...
app.get('/api/quejas', authenticateToken, async (req, res) => {
    try {
        ...
        let query = `SELECT * FROM vista_quejas_unificada`;
        let whereConditions = [];
        let queryParams = [];
        let paramIndex = 1;
        if (estado) {
        ...
        res.status(200).json({
            success: true,
            data: result.rows,
            pagination: { page: pageNum, limit: limitNum, total: result.rows.length }
        });
    } catch (error) {
...
app.get('/health', (req, res) => {
    res.status(200).json({ status: 'ok', timestamp: new Date().toISOString(), version: '6.0' });
});
...
app.use((error, req, res, next) => {
    logger.error('ERROR NO MANEJADO:', { error: error.message, stack: error.stack, url: req.originalUrl, method: req.method, ip: req.ip });
